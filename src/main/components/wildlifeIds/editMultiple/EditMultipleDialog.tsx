...
const EditMultipleDialog = ({open, close, acceptAction, noun, title}) => {
	const {purpose: purposes} = useSelector(state => state.CodeTables.tables);
			...
			case 'status':
				return (
					<Grid item container rowGap={4} mt={2}>
						...
						<Grid item xs={12}>
							<TextField label="Reason (Enter a reason why you are changing the WLH ID status)" id="reason" multiline rows={3} />
						</Grid>
					</Grid>
				);
				break;
